...
constructor(
    ...
    validateInput(index:number, that:RegisterService) {
      let result = true;
      if (this.data[index] !== '' && this.regex[index].test(this.data[index])) {
    ...
    validateAll(that:RegisterService) {
      let result = -1;
  ...
  checkUserName(that:RegisterService) {
    // const that = this;
    const param = {
    ...
    return that.requestService.get(url, param).then((rs: string | any[]) => { // 从远程服务器取用户名
      if (rs.length !== 0) {
    ...
    check2password(that:RegisterComponent): boolean {
        let result = true;
        if (that.rs.model.data[1] === that.rs.model.data[2]) {
          that.rs.model.info[2]  = 'Pass';
          that.rs.model.color[2] = 'green';
        } else {
          if ( +that.rs.model.data[2].length > 0) {
            that.rs.model.info[2]  = 'Two passwords are different';
            that.rs.model.color[2] = 'red';
            that.logonInputs._results[2].nativeElement.select();
          } else {
            that.rs.model.info[2]  = 'Re-enter password';
            that.rs.model.color[2] = '#999';
          }
          result = false;
        }
        return result;
